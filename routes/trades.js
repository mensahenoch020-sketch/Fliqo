...
router.post('/submit', protect, upload.single('cardImage'), async (req, res) => {
  try {
    ...
    if (!ratePerDollar)
      return res.status(400).json({ success: false, message: 'Rate unavailable.' });

    const nairaAmount = Math.round(parseFloat(denomination) * ratePerDollar);

    let cardImageUrl = null;
    if (req.file) {
      // In production: upload to Cloudinary, store URL
    ...
    const trade = await Trade.create({
      ...
      cardCode, cardImageUrl, ratePerDollar, nairaAmount,
      status: 'pending', verificationStarted: new Date()
    });
...
router.post('/:id/dispute', protect, async (req, res) => {
  try {
    ...
    trade.adminNote = `Disputed by user: ${reason}`;
    await trade.save();

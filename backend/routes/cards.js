...
// POST /api/cards/rate-alert
router.post('/rate-alert', protect, async (req, res) => {
  try {
    const { cardType, targetRate } = req.body;
    if (!cardType || !targetRate)
      return res.status(400).json({ success: false, message: 'cardType and targetRate required.' });
    const user = await User.findById(req.user._id);
    if (user.rateAlerts.filter(a => a.isActive).length >= 10)
      return res.status(400).json({ success: false, message: 'Maximum 10 active alerts allowed.' });
...
// DELETE /api/cards/rate-alert/:alertId
...
router.post('/kyc', protect, async (req, res) => {
  try {
    ...
    if (!bvn && !nin)
      return res.status(400).json({ success: false, message: 'BVN or NIN required.' });
    const user = await User.findById(req.user._id);

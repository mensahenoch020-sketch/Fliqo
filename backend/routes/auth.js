...
const sendOTP = async (phone, otp) => {
  ...
  try {
    ...
    await twilio.messages.create({
      body: `Your Fliqo verification code: ${otp}. Valid for 10 minutes. Do not share.`,
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (user.otpExpires < Date.now()) return res.status(400).json({ success: false, message: 'OTP expired. Request a new one.' });

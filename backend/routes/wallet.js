...
router.post('/withdraw', protect, async (req, res) => {
  try {
    ...
    if (!bank)
      return res.status(400).json({ success: false, message: 'No bank account found. Please add one.' });
    ...
    // Flutterwave payout — only runs if real keys are configured
    const flwKey = process.env.FLW_SECRET_KEY;
    if (flwKey && !flwKey.includes('your_secret_key_here')) {
      try {
      ...
      console.log(`[DEV MODE] Withdrawal: ₦${withdrawAmount} → ${bank.bankName} ${bank.accountNumber}`);
    }
...
// POST /api/wallet/withdrawal-callback
router.post('/withdrawal-callback', async (req, res) => {
  console.log('FLW Webhook received:', req.body);
  res.sendStatus(200);
});
